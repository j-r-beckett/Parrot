...
git -C "$DIR" ls-files | while read -r FILE; do
  ...
  # Skip excluded files
  if [[ "$FILE" == "copy-repo.sh" || "$FILE" == ".editorconfig" || "$FILE" == ".gitignore" || "$FILE" == "uv.lock" || "$FILE" == ".env" || "$FILE" == "notes.md" ]]; then
    echo "Skipping excluded file: $FILE"
    continue
  fi
  
  # Skip entire .venv directory
  if [[ "$FILE" == .venv/* ]]; then
    echo "Skipping .venv directory file: $FILE"
    continue
  fi
  
  # Skip __pycache__ directories
  if [[ "$FILE" == *"__pycache__"* ]]; then
    echo "Skipping __pycache__ file: $FILE"
    continue
  fi

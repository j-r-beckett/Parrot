...
class CitiBikeClient:
    ...
    def __init__(self, httpx_client: httpx.AsyncClient, system_logger):
        ...
        self._station_status: List[StationStatus]
        self._station_info: List[StationInfo]
    ...
    async def __aenter__(self) -> Self:
        ...
        if version != CitiBikeClient.api_version:
            raise ValueError(f"API version mismatch: expected {CitiBikeClient.api_version}, got {version}")

        self._station_status, _ = await self._update_station_status()
        self._station_info, _ = await self._update_station_info()
    ...
    def _next_update_time(self, last_updated: int, ttl: int) -> datetime:
        ...
        last_updated_dt = datetime.fromtimestamp(last_updated, tz=self.timezone).astimezone(timezone.utc)
    ...
    async def _sync(
        self,
        updater: Callable[[], Awaitable[Tuple[List[StationInfo] | List[StationStatus], datetime]]],
    ) -> None:
        while True:
            try:
    ...
    async def get_stations(self) -> List[Station]:
        ...
        for station_id, (lat, lon) in info_dict.items():
            if station_id in status_dict:
                ...
                self.system_logger.error(f"Station {station_id} found in info but not in status")

...
usage() {
    ...
    echo "Start sms-proxy service with system configuration"
...
# Start sms-proxy
echo "[$(date)] Starting sms-proxy service" >> $LOG
cd "$SCRIPT_DIR" && setsid ./sms-proxy -port "${SMS_PROXY_PORT}" -private-ip "${PRIVATE_IP}" >> sms-proxy.log 2>&1 < /dev/null &
echo "[$(date)] sms-proxy started, logging to $SCRIPT_DIR/sms-proxy.log" >> $LOG

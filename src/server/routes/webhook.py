...
async def handle_smsgap_received(request: Request, state: State, data: SmsReceived) -> str:
    ...
    # Send response back via smsgap (settler phone)
    await send_sms_smsgap(
        state.settler_smsgap_client,
        response,
...
async def handle_smsgap_delivered(request: Request, state: State, data: SmsDelivered) -> str:
